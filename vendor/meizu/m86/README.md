...
This tree contains generated build definitions, not stock binaries.
`proprietary/` is intentionally ignored by the local Git repository. The
authoritative extraction scripts and list live under `device/meizu/m86` so
LineageOS can safely clean and regenerate this directory. Stock bytes are
transferred to the builder only through the explicit proprietary-blob
workflow.
...
Run `device/meizu/m86/extract-files.sh PATH_TO_EXPANDED_ROM` only from an
installed LineageOS 17.1 checkout so the branch-matched `extract_utils.sh`

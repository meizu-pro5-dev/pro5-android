...
usage() {
  printf 'Usage: %s BASE_V2_RECOVERY_IMG OUTPUT_DIRECTORY\n' \
    "$(basename "$0")" >&2
}
...
init_patch="$project_root/patches/twrp-diagnostics/0003-run-recovery-under-log-wrapper.patch"
recovery_wrapper="$project_root/patches/twrp-diagnostics/recovery-log-wrapper.sh"
...
expected_base_ramdisk_sha256=5e97bb668101cf85037ac276e25ed76d6d40a99cc08e73150183fa1b98ef4ca4
expected_base_init_sha256=469da9ca941cabec0d42ec7681b47988b0fb38711bb58f5af227a41c18a24335
expected_patched_init_sha256=f1e17f7d93dad91efa607098495debda771429a8ba326c0ab6ba9e883811ee70
expected_init_patch_sha256=eb13bb76987cd2055e7ac8b67ee3d6e0369b49b0b65188c5b6801efb7bb9032b
expected_original_permissive_sha256=d87ec082c473d92f5b9b69e0e55d967c7327a07415610f9ab1dc60a7b209d577
expected_wrapper_sha256=fa1b422d4e5a92ba833a6d0458413be656bf5800537b22628d95625bc6b388b4
...
for required_file in \
  ...
  if [[ ! -s "$required_file" ]]; then
    printf 'Required v3 build input is absent: %s\n' "$required_file" >&2
    exit 1
  fi
done
...
if [[ ! -f "$base_input" ]]; then
  printf 'The v2 base image is absent: %s\n' "$base_input" >&2
  exit 1
fi
...
require_sha256 "$expected_init_patch_sha256" "$init_patch"
require_sha256 "$expected_wrapper_sha256" "$recovery_wrapper"
...
stage_root="$(mktemp -d "$output_parent/.pro5-twrp-v3.XXXXXX")"
cleanup() {
  if [[ -n "${stage_root:-}" && -d "$stage_root" && \
      "$stage_root" == "$output_parent/.pro5-twrp-v3."* ]]; then
    rm -r -- "$stage_root"
  fi
}
trap cleanup EXIT
...
mkdir -p "$patch_root" "$pass_one" "$pass_two" \
...
python3 "$boot_image_inspector" "$base_image" \
  ...
  --expect-ramdisk-file-sha256 \
    "sbin/permissive.sh=$expected_original_permissive_sha256" \
  --expect-ramdisk-file-sha256 \
  ...
  --extract-ramdisk-file "init.rc=$patch_root/init.rc" \
...
(
  ...
  git init -q
  git add init.rc
  git apply --check "$init_patch"
  git apply "$init_patch"
  git apply --reverse --check "$init_patch"
  git diff --check
  if [[ "$(git diff --name-only)" != "init.rc" ]]; then
    printf 'The v3 init patch changed an unexpected path.\n' >&2
    exit 1
  fi
  git diff --no-ext-diff -- init.rc > \
    "$staged_artifact/INIT-RC-DIFF.patch"
)
require_sha256 "$expected_patched_init_sha256" "$patch_root/init.rc"
...
for pass_spec in "ONE:$pass_one" "TWO:$pass_two"; do
  ...
  python3 "$ramdisk_rewriter" \
    ...
    --replace-data "init.rc=$patch_root/init.rc" \
...
if ! cmp --silent "$pass_one/ramdisk.gzip" "$pass_two/ramdisk.gzip" || \
  ...
  printf 'The two clean v3 repacks are not byte-identical.\n' >&2
  exit 1
fi

cp "$pass_one/ramdisk.gzip" "$staged_artifact/ramdisk.gzip"
cp "$pass_one/recovery.img" "$staged_artifact/recovery.img"
cp "$patch_root/init.rc" "$staged_artifact/init.rc"
cp "$init_patch" \
  "$staged_artifact/provenance/0003-run-recovery-under-log-wrapper.patch"
cp "$recovery_wrapper" \
  "$staged_artifact/provenance/recovery-log-wrapper.sh"
...
cp "${BASH_SOURCE[0]}" \
  "$staged_artifact/provenance/build-pro5-twrp-log-capture-v3.sh"
...
python3 "$boot_image_inspector" "$staged_artifact/recovery.img" \
  ...
  --expect-ramdisk-compression gzip \
  --expect-ramdisk-file-sha256 \
    "init.rc=$expected_patched_init_sha256" \
  --expect-ramdisk-file-sha256 \
    "init.recovery.m86.rc=$expected_device_init_sha256" \
  --expect-ramdisk-file-sha256 \
...
{
  printf 'artifact_role=test-only persistent TWRP log-capture diagnostic v3\n'
  ...
  printf 'base_device_result=visible failure repeated; recovery log not retained\n'
  printf 'changed_data_paths=init.rc,sbin/permissive.sh\n'
  ...
  printf 'patched_init_sha256=%s\n' "$expected_patched_init_sha256"
  ...
  printf 'persistent_write=mount cache read-write and create unique pro5-twrp-diag-v3 files under /cache/recovery\n'
  printf 'persistent_sync_interval_seconds=2\n'
...

{
  ...
  printf 'init_patch_sha256=%s\n' "$expected_init_patch_sha256"
  printf 'wrapper_sha256=%s\n' "$expected_wrapper_sha256"
  printf '\nworktree_status:\n'
...
# PRO 5 persistent TWRP log-capture diagnostic v3

This image is a test-only evidence collector, not an accepted recovery. It is
built from the exact tested v2 image and keeps its proven kernel, patched TWRP
ELF, dependencies, resources, fstab and boot geometry. Only the data payloads
of \`init.rc\` and the existing executable \`sbin/permissive.sh\` entry change;
all 489 cpio paths, path order and metadata remain identical.

The new recovery service runs TWRP as a child of a persistent wrapper. Before
TWRP starts, the wrapper mounts the existing cache filesystem read-write,
creates unique \`/cache/recovery/pro5-twrp-diag-v3-*\` files, redirects
\`/tmp/recovery.log\` to the persistent log, captures an initial dmesg and
syncs cache every two seconds. If TWRP returns, the wrapper records its exit
status and a second dmesg, then remains alive. If cache cannot be mounted or
written, TWRP is not started and the init/ADB environment is held for review.
...
approved. The test deliberately writes diagnostic files to \`/cache/recovery\`.
It must never be written to \`bootimg\`, \`dtb\`, \`/dev/block/sdb\`, firmware,
parameter or identity partitions. Do not mount, wipe, format, install, restore
or perform any other recovery write test.

After a test, collect every \`pro5-twrp-diag-v3-*\` file before changing the
recovery image. \`*.log\` is TWRP's log, \`*-wrapper.log\` records wrapper and
pre-freopen process output, and the dmesg files bracket TWRP execution. If
recovery ADB is reachable, \`/tmp/recovery.log\` points at the same persistent
log. If the handset returns to Flyme, use the already established rooted,
read-only collection route for those cache files.
EOF
...
printf 'Built PRO 5 TWRP persistent log-capture v3.\n'
printf 'artifact_dir=%s\n' "$artifact_dir"
printf 'recovery_size=%s\n' "$artifact_size"
printf 'recovery_sha256=%s\n' "$artifact_sha256"
printf 'persistent_device_write=/cache/recovery/pro5-twrp-diag-v3-*\n'

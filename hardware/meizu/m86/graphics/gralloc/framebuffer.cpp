...
static int fb_post(struct framebuffer_device_t* dev, buffer_handle_t buffer)
{
...
#if HWC_EXIST
    hwc_callback_queue_t* queue =
            reinterpret_cast<hwc_callback_queue_t*>(m->queue);
    ...
    // This Exynos mapper returns up to three plane addresses through vaddr.
    void* buffer_vaddr[3] = {};
...
int init_fb(struct private_module_t* module)
{
    ...
    module->framebuffer->base = reinterpret_cast<size_t>(vaddr);
                   ...
                   hw_device_t** device)
{
    ...
    framebuffer_device_t* dev =
            static_cast<framebuffer_device_t*>(malloc(sizeof(framebuffer_device_t)));
    if (dev == nullptr) {
    ...
    dev->setSwapInterval = nullptr;
    dev->post = fb_post;
    dev->setUpdateRect = nullptr;
    dev->compositionComplete = nullptr;
#if HWC_EXIST
    m->queue = new hwc_callback_queue_t;
#endif
    pthread_mutex_init(&m->queue_lock, nullptr);
